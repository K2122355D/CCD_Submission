...
def index():
    if request.method == "POST":
        ...
        s2 = "The score of credit card upgrade base on decision tree is "+ str(pred2)
        model3 = joblib.load('CCU_NN')
        pred3 = model3.predict([[purchases, suppcard]])
        s3 = "The score of credit card upgrade base on decision tree is "+ str(pred3)
        model4 = joblib.load('CCU_Reg')
        pred4 = model4.predict([[purchases, suppcard]])
        s4 = "The score of credit card upgrade base on decision tree is "+ str(pred4)
        model5 = joblib.load('CCU_RF')
        pred5 = model5.predict([[purchases, suppcard]])
        s5 = "The score of credit card upgrade base on decision tree is "+ str(pred5)
        return(render_template("index.html", result1 =s1, result2 ="1", result3 ="1", result4="1", result5 ="1"))
    else:
